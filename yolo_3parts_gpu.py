"""
修复版（GPU加速）：大幅扩展台面区域，确保覆盖整个桌面/台面
"""
...
# ==================== 配置区 ====================
VIDEO_PATH = r"C:\Users\Administrator\Desktop\sh(final)\video\3parts_Test2.mp4"
OUTPUT_DIR = r"C:\Users\Administrator\Desktop\sh(final)\output\yolo_3parts"

SEG_MODEL_PATH = r"C:\Users\Administrator\Desktop\sh\models\yolo\yolo11n-seg.pt"
DET_MODEL_PATH = r"C:\Users\Administrator\Desktop\sh\models\yolo\yolo11n.pt"
...
if torch.cuda.is_available():
    ...
    print(f"当前使用设备: {DEVICE} (GPU)")
else:
    print(f"当前使用设备: {DEVICE} (CPU)")
print(f"=" * 60)
# ==============================================
...
def create_table_mask_from_scene(height, width, det_results):
    ...
    for box in det_results.boxes:
        ...
        # 这些物体通常放在台面上
        if class_name in ["laptop", "mouse", "keyboard", "book", "cup", "bottle", "cell_phone", "remote", "sink", "toilet", "hair_drier"]:
...
def detect_regions_and_objects(video_path, output_dir):
    ...
    seg_model.to(DEVICE)  # 显式移动到GPU
    print(f"分割模型已加载到: {next(seg_model.parameters()).device}")

    print(f"加载检测模型: {DET_MODEL_PATH}")
    det_model = YOLO(DET_MODEL_PATH)
    det_model.to(DEVICE)  # 显式移动到GPU
    print(f"检测模型已加载到: {next(det_model.parameters()).device}")
    ...
    while True:
        ...
        # ========== Step 1: 分割区域（GPU加速）==========
        seg_results = seg_model(frame, verbose=False, conf=CONF_THRESHOLD, imgsz=IMG_SIZE, device=DEVICE)[0]
        ...
        # ========== Step 2: 检测物体（GPU加速）==========
        det_results = det_model(frame, verbose=False, conf=CONF_THRESHOLD, imgsz=IMG_SIZE, device=DEVICE)[0]
